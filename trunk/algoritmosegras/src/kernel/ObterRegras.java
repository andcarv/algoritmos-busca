...
public abstract class ObterRegras {
	...
	//SortedSet<Double> limiares; // limiares utilizados na constru��o do gr�fico ROC
...
//		System.out.println("limiares.size = " + limiares.size());
	...
	public void obterRankingsVotacao(Instances dadosTeste, ArrayList<Regra> regras, ArrayList<InstanceVotacao> votos, TreeSet<Double> limiares){
...
//					Se o valor real do exemplo for negativo
	...
	/**
	 * M�todo que preenche a matriz de confus�o multi classe de acordo com as instancias e regras passadas por parametro
	 * @author matheus
	 * @param confusao Matriz de confusao a ser preenchida
	 * @param dadosTeste Dados de testes
	 * @param regras Regras do modelo
	...
	public void preencherMatrizConfusaoMultiClasse(MatrizConfusaoMultiClasse confusao, Instances dadosTeste, ArrayList<Regra> regras){
		...
		for(int i = 0; i < dadosTeste.numInstances(); i++){
			...
			//em caso de n�o haver empate pega-se o primeiro e �nico elemento do arraylist que � a classe eleita
 ...
 */
			...
			for(int i = 0; i<numFolds; i++){
				...
				if(regras.isEmpty()){
					...
					if(AUC){
						...
						//Como n�o h� c�lculo das medidas, n�o h� o processod e vota��o.
						//Assim a op��o sele��o na grava��o de regras deve ser sempre falsa
	 ...
	 * J� esta divindo o base de treinamento em partes iguais, executando o algoritmo para todas as parti��es, juntando as regras
	 * deixando apenas as regras n�o dominadas. Falta a parte do c�lculo da AUC no arquivo de teste.
	 * @deprecated Metodo com problema. A versão nova do Weka não possui o método resampleWithWeights(dadosTreinamentoTotal, new Random(), sampled);
	 * @param nomeBase
	 * @param caminhoBase
	 * @param nomeMetodo
	 * @param cPositiva
	...
	/**
	 * M�todo que grava as regras da execucao em arquivo texto e adiciona as regras numa lista
...
/**
 * M�todo que executa o treinamento da algoritmo passado como parametro no arquivo principal
...
/**
 * M�todo que instania a votacao de acordo com a definicao do arquivo de configuracao
 * @param votacao
 */
