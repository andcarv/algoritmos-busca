...
/**
 * Classe que guarda informações de todo o conjunto de execuções. Coleção de
 * DadosFold
 * 
 * @author andre
 *
...
public class DadosExperimentos {
	...
	public void calcularMediaAreasPrecisaoNumRegras() {
		...
		for (Iterator<DadosExecucao> iter = dadosFolds.iterator(); iter
				.hasNext();) {
	...
	/**
	 * Método que gera dois arquivos um contendo as medidas (AUC, Accuracy,
	 * Precison, Recall, F-Measure) e outro contendo os comandos para carregar
	 * vetores na ferramenta R
	 * 
	 * @param caminhoSaida
	 *            Caminho da saida dos arquivos
	 * @param arquivoMedidas
	 *            Nome do arquivo com as medidas
	 * @param arquivoComandos
	 *            Nome do arquivo com os comandos
	 * @param ID
	 *            String utilizada para diferenciar vários dados de uma mesma
	 *            execucao
	 * @throws Exception
	 */
	public void gerarArquivosMedidas(String caminhoSaida,
			String arquivoMedidas, String arquivoComandos,
			String arquivoConfusao) throws Exception {
		PrintStream med = new PrintStream(caminhoSaida + arquivoMedidas + ID
				+ ".txt");
		PrintStream com = new PrintStream(caminhoSaida + arquivoComandos + ID
				+ ".txt");
		PrintStream conf = new PrintStream(caminhoSaida + arquivoConfusao + ID
				+ ".txt");
	...
	public String toString() {
		...
		for (Iterator<DadosExecucao> iter = dadosFolds.iterator(); iter
				.hasNext();) {
			DadosExecucao fold = (DadosExecucao) iter.next();
			str.append("Fold: " + fold.numExec + "-" + fold.numFold + "\t"
					+ fold.toString() + "\n");
		}
		...
		if(i==0){
		try {
			escritor("SaidaTestes.txt",media.toString().replace('.', ',')+"	"+precisao.toString().replace('.', ',')+"\n");
		} catch (IOException e) {
		...
		str.append("Desvio Padrao:\t" + desvio.toString().replace('.', ',')
				+ "\n");
		str.append("Número médio de regras:\t"
				+ numRegras.toString().replace('.', ',') + "\n");
		str.append("Media das Precisoes:\t"
				+ precisao.toString().replace('.', ',') + "\n\n");
		/*
		 * for (Iterator iter = dadosFolds.iterator(); iter.hasNext();) {
		 * DadosFold fold = (DadosFold) iter.next(); str.append("Fold: " +
		 * fold.numFold + "\n"); str.append(fold.confusao + "\n\n"); }
		 */
		return str.toString();
	}

	public void escritor(String path, String parametro)
			throws IOException {
