...
public class Principal {
				...
				public void run() {
					...
					try {
						...
						if (principal.metodo.equals("pso"))
							principal.algoritmo = new NuvemParticulas(
									principal.geracoes, principal.populacao,
									principal.objetivos, principal.combinacao);
						else
							principal.algoritmo = new ObterRegrasApriori(
									principal.numRegras, principal.confianca,
									principal.minSuporte, principal.maxSuporte,
									principal.delta,
									principal.fronteiraApriori,
									principal.objetivos);

						if (principal.medoid)
							principal.algoritmo.executarMedoid(
									principal.nomeBase, principal.caminhoBase,
									principal.metodo, principal.numclasses,
									principal.classePositiva,
									principal.classeNegativa,
									principal.numFolds, principal.numExec,
									principal.nomeBase, principal.auc,
									principal.verbose, principal.votacao,
									principal.selecaoVotacao);
						else {
							if (principal.numParticoes == 1)
								principal.algoritmo.executarFolds(
										principal.nomeBase,
										principal.caminhoBase,
										principal.metodo, principal.numclasses,
										principal.classePositiva,
										principal.classeNegativa,
										principal.numFolds, principal.numExec,
								...
								principal.algoritmo.executarParalelo(
										principal.nomeBase,
										principal.caminhoBase,
										principal.metodo, principal.numclasses,
										principal.classePositiva,
	...
	/**
	 * M�todo que carregar as informa��es da execu��o a partir de um
	 * arquivo de configura��o nomebase = Base a ser executada caminhoBase =
	 * Caminho do diretorio da base metodo = M�todo que ser� a base ser�
	 * executada (pso, apriori) classePositiva = �ndice da classe positiva (0
	 * ou 1) classeNegativa = �ndice da classe positiva (1 ou 0) numClasses =
	 * N�mero de classes da base numFolds = N�mero de folds da base de dados
	 * numExec = N�mero de execu��es do algoritmo Parametros da nuvem de
	 * particulas geracoes = N�mero de itera��es do problema populacao =
	 * N�mero inicial de part�culas Parametros do apriori numRegras =
	 * N�mero m�ximo de regras gerado pelo algoritmo confianca = Confianca
	 * m�nima minSuporte = Suporte m�nimo maxSuporte = Suporte m�ximo
	 * delta = Taxa de varia��o do suporte auc = Executa ou n�o o
	 * c�lculo da AUC
	 * 
	 * @param nomeArquivo
	 * @throws IOException
	...
	public void carregarArquivoConf(String nomeArquivo) throws IOException {
		...
		while (buff.ready()) {
			...
			if (linha.length != 2) {
				System.err
						.println("Erro no arquivo de configura��o! Linha: "
								+ linhaString);
				System.exit(0);
			}
